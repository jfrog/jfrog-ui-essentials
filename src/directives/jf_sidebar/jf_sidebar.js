class jfSidebarController {
	...
	_init() {
		...
		$('body').on('keydown', (e) => {
			...
			if ((e.keyCode === 27 || (e.keyCode === 37 && e.ctrlKey && e.altKey)) && this.menu.width === this.subMenuWidth && $(
					'.sub-menu').length > 0) {
				this.$timeout(() => {
	...
	mouseOverMenu() {
		...
		if (this.menu.width === this.subMenuWidth && $('.menu-item:hover').length && $(
				'a.menu-item.extended-item:hover').length < 1) {
			...
			if (!angular.isDefined(this.openMenu)) {
				this.openMenu = this.$timeout(() => {
					let widthToOpen = ($('.sub-menu').length > 0 && $(
						'a.menu-item.extended-item:hover').length) ? this.subMenuWidth : '200px';
					if (($('.sub-menu:hover').length || $(
							'.menu-item.extended-item:hover').length) && angular.isDefined(this.openMenu)) {
	...
	mouseLeaveMenu(e) {
		if (e && e.toElement) {
			if ($(e.toElement).hasClass('tooltipster-content') || $(e.toElement.parentElement).hasClass(
					'tooltipster-arrow')) {
	...
	closeSubMenu(delay, force = false, expand = false) {
		...
		if (!force && ($('.sub-menu:hover').length || $('.menu-item.extended-item:hover').length || ($('.sub-menu')
				.find('a')
				.is(':focus') || ($('#menuSearchQuery').is(':focus') && $('#menuSearchQuery').val().length > 0)))) {
			...
			this.subMenuItemDelayTimer = this.$timeout(() => {
				...
				delete this.subMenuItemDelayTimer;
				if (!$('.sub-menu:hover').length && !$('.tooltipster-content:hover').length && !$(
						'.tooltipster-arrow:hover').length) {
		...
		if (elem.length) {
			if (this.driver.onBeforeStateSwitch) {
				this.driver.onBeforeStateSwitch(
					{state: elem.data('state'), stateParams: elem.data('params')});
			}
