  ...
  <div>
    <div class="grid-batch-actions text-right">
      <div
        ...
        v-jf-tooltip.bind="isDisabled(action) ? action.getDisabledTooltip() : ''"
        class="grid-batch-action-wrapper"
        :class="{'more-actions': action === MORE_ACTIONS}"
        :data-action-id="action.name"
        :style="{visibility: action === MORE_ACTIONS || visibleActions.indexOf(action) !== -1 ? 'inherit' : 'hidden', opacity: action === MORE_ACTIONS || visibleActions.indexOf(action) !== -1 ? 1 : 0}"
      >
        <a
          v-if="action !== MORE_ACTIONS && (!action.visibleWhen || action.visibleWhen())"
          :class="{disabled: isDisabled(action), ['btn batch-action-' + action.icon]: true}"
        ...
        >
          ...
          <i
            v-if="!action.img"
            :class="'icon icon-' + action.icon"
          /> {{ action.name }}
        </a>
        <div
          v-if="action === MORE_ACTIONS"
          class="more-wrapper"
        >
          <jf-actions
            :parent-controller="jfBatchActions"
            label="More Actions"
